"""
MCP GitHub Server with Server-Sent Events (SSE) endpoint.
"""
import json
import os
import time
...
GITHUB_TOKEN = os.getenv("GITHUB_TOKEN", "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")
if not GITHUB_TOKEN:
...
def generate_mcp_message_stream():
    ...
    # FIXED: Properly escape braces in f-string
...
def repo_events(owner, repo):
    ...
    def event_stream():
        ...
        # FIXED LINE: Completely rewrote the f-string to ensure proper brace escaping
        # This avoids the SyntaxError: f-string: single '}' is not allowed
        data = {
            "jsonrpc": "2.0",
            "method": "ready",
            "id": f"{server_id}-ready",
            "params": {
                "time": now,
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    # FIXED: Using json.dumps for complex nested structures instead of f-string
                    data = {
                        "jsonrpc": "2.0",
                        "method": "repo_update",
                        "id": f"update-{message_id}",
                        "params": {
                            "time": now,
